...
customicon <- leaflet::makeIcon(
  iconUrl = "inst/app/www/ambulance.png",
...
#' @import dplyr
#'
datasetprop <- function(dataset) {
  months <- dataset %>% group_by(month(DATE)) %>% tally()
  weeks  <- dataset %>% group_by(Week) %>% tally()
  dtacomp = ifelse(nrow(months) > 1,"Monthly,Weekly,Daily",
  ifelse(nrow(weeks)>1,"Weekly,Daily","Daily"))
  return(unlist(strsplit(dtacomp,",")))
}
